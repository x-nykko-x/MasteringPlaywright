...
test("Login and create the storage state json file", async ({ page }) => {
  ...
  await loginPage.fillUsername(decrypt(process.env.SALESFORCE_USER!));
  await loginPage.clickLoginButton();
  await loginPage.fillPassword(decrypt(process.env.SALESFORCE_PASSWORD!));
  await loginPage.clickLoginButton();
...
test("Login with auth file", async ({ browser }) => {
  ...
  await page.goto("https://orgfarm-18068347c3-dev-ed.develop.lightning.force.com/lightning/page/home");
